...
| `initialSortModel` | `Array<{ field: string, order: 'asc' \| 'desc' }>` | Default client sort whenever there is **no meaningful persisted user sort**: missing key, stored `[]`, invalid JSON, or stored entries that all drop out after validation (e.g. column removed). Entries are ignored if `field` is not a column or `order` is not `asc`/`desc`. A **non-empty** saved model always wins on load. **Clear sort** returns to this default when provided (otherwise `[]`). The usual `saveSortModel` effect persists whatever is active, so after clear the stored value may become the sanitized default. Without `gridId`, applies on first render in memory only (sort is not persisted). |
